...
class Restart(Command):
    name = ["Shutdown", "Reboot", "Update","재시작"]
    min_level = nan
    description = "입력한 딜레이 후  Miza 봇을 재시작, 리로드 혹은 종료한다."
    usage = "<딜레이>?"
    _timeout_ = inf
...
class Execute(Command):
    name=["실행"]
    min_level = nan
    description = "마인크래프트의 명령 기능과 유사하게 다른 사용자로 명령을 실행한다 "
    usage = "as <0:유저>* run <1:명령어>+"
    multi = True
...
class Exec(Command):
    name = ["Eval","평가"]
    min_level = nan
    description = "현재 채널에 있는 봇 소유자의 모든 메시지가 MIZA에서 파이썬 코드로 실행됩니다."
    usage = "(명령어 사용 권한(가능/불가능))? <type(virtual)>?"
    flags = "aed"
...
class Immortalise(Command):
    name = ["Immortalize","영구삭제","URL삭제"]
    min_level = nan
    description = "대상 웹 서버 URL을 영구 삭제한다"
    usage = "<이미지url>"
...
class SetAvatar(Command):
    name = ["ChangeAvatar", "UpdateAvatar","아바타설정","아바타변경"]
    min_level = nan
    description = "MIZA의 현재 아바타를 변경한다."
    usage = "<아바타 url>?"
...
class Suspend(Command):
    name = ["Block", "Blacklist","블랙리스트"]
    min_level = nan
    description = "사용자를 블랙리스트에 추가하여 MIZA에 대한 명령 권한을 제거합니다."
    usage = "<0:유저> <disable(?d)>"
    flags = "aed"

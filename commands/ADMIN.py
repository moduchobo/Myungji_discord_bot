...
class Purge(Command):
    time_consuming = True
    _timeout_ = 16
    name = ["🗑", "Del", "Delete", "Purge_Range","삭제","제거"]
    min_level = 3
    description = "현재 채널에서 특정 사용자의 다수의 메세지를 삭제한다."
    usage = "<1:유저>? <0:개수(1)>? <무시{?i}|범위{?r}|숨기기{?h}>*" #user와 메세지 개수를 입력하고 ignore,range,hide 기능중에 한개를 (?알파벳) 형태로 입력한다.
    flags = "fiaehr"
    rate_limit = (2, 4)
    multi = True
    slash = True
...
class Mute(Command):
    server_only = True
    _timeout_ = 16
    name = ["🔇", "Revoke", "Silence", "UnMute","음소거"]
    min_level = 3
    min_display = "3+"
    description = "선택적인 이유로 특정 시간 동안 사용자를 음소거한다."
    usage = "<0:유저>* <1:시간>? (이유)? <2:이유>? <숨기기{?h}>?" #유저와 시간을 입력한 뒤 이유 입력 또는 hide 사용하려면 ?h입력
    flags = "fhz"
...
class Ban(Command):
    server_only = True
    _timeout_ = 16
    name = ["🔨", "Bans", "Unban","밴","금지"]
    min_level = 3
    min_display = "3+"
    description = "선택적인 이유로 특정 시간 동안 사용자를 금지한다."
    usage = "<0:유저>* <1:시간>? (이유)? <2:이유>? <숨기기{?h}>?"#유저와 시간을 입력한 뒤 이유 입력 또는 hide 사용하려면 ?h입력
    flags = "fhz"
    directions = [b'\xe2\x8f\xab', b'\xf0\x9f\x94\xbc', b'\xf0\x9f\x94\xbd', b'\xe2\x8f\xac', b'\xf0\x9f\x94\x84']
    dirnames = ["First", "Prev", "Next", "Last", "Refresh"]
...
class RoleSelect(Command):
    server_only = True
    name = ["ReactionRoles", "RoleButtons", "RoleSelection", "RoleSelector","역할버튼","역할선택"]
    min_level = 3
    min_display = "3+"
    description = "사용자가 지정된 목록에서 역할을 자체 할당할 수 있는 메시지를 만든다."
    usage = "<역할>+"
    flags = "ae"
    no_parse = True
    rate_limit = (1, 2)
...
class RoleGiver(Command):
    server_only = True
    name = ["Verifier","자동역할부여자","검증자"]
    min_level = 3
    min_display = "3+"
    description = "현재 채널에 자동 역할 부여자를 추가한다. 메시지의 키워드에 의해 작동되며 권한 수준이 >= 0인 사용자에게만 적용된다."
    usage = "<0:반응대상>? <1:역할>? <메세지 삭제{?x}>? <비활성화{?d}>?"
    flags = "aedx"
    no_parse = True
    rate_limit = (2, 4)
    slash = True
...
class AutoRole(Command):
    server_only = True
    name = ["InstaRole","자동역할부여","자동역할"]
    min_level = 3
    min_display = "3+"
    _timeout_ = 12
    description = "서버에 가입하는 모든 새 사용자가 자동으로 대상 역할을 얻도록 한다. 여러 역할을 입력하여 임의 역할 부여자를 만든다."
    usage = "<역할>? <모두 업데이트{?x}>? <비활성화{?d}>?"
    flags = "aedx"
    rate_limit = 1
    slash = True
...
class RolePreserver(Command):
    server_only = True
    name = ["🕵️", "StickyRoles","역할기억","역할저장기"]
    min_level = 3
    min_display = "3+"
    description = "<MIZA>가 모든 사용자에 대한 역할을 저장하고 사용자가 나갔다가 다시 가입할 때 해당 역할을 다시 추가한다."
    usage = "(활성화|비활성화)?"
    flags = "aed"
    slash = True
...
class NickPreserver(Command):
    server_only = True
    name = ["StickyNicks", "NicknamePreserver","별명저장기","별명기억"]
    min_level = 3
    min_display = "3+"
    description = "<MIZA>가 모든 사용자의 별명을 저장하고 사용자가 나갔다가 다시 가입할 때 다시 추가한다."
    usage = "(활성화|비활성화)?"
    flags = "aed"
...
class ThreadPreserver(Command):
    server_only = True
    name = ["KeepAlive", "ThreadBump", "AutoBump", "UnArchive","스레드범프","부활","자동부활"]
    min_level = 3
    min_display = "3+"
    description = '자동 보관 시 <MIZA>가 현재 스레드를 "범프"(부활)하도록 한다.'
    usage = "(활성화|비활성화)?"
    flags = "aed"
...
class Lockdown(Command):
    ...
    name = ["🔒", "☣️"]
    min_level = inf
    description = "모든 사용자에 대한 발송 메시지 권한을 제거하고 모든 초대를 취소하며 모든 스레드를 보관하여 서버를 완전히 잠근다."
    flags = "f"
    rate_limit = 30
...
class SaveChannel(Command):
    time_consuming = 1
    _timeout_ = 16
    name = ["BackupChannel", "DownloadChannel","채널백업","채널다운로드"]
    min_level = 3
    description = "채널의 여러 메시지와 해당 내용을 .txt 파일에 저장한다."
    usage = "<0:채널>? <1:메세지_제한(4096)>?"
...
class UserLog(Command):
    server_only = True
    name = ["MemberLog","멤버기록","사용자기록"]
    min_level = 3
    description = "<MIZA>가 서버의 사용자 및 구성원 이벤트를 현재 채널에 기록한다."
    usage = "(활성화|비활성화)?"
    flags = "aed"
    rate_limit = 1
...
class MessageLog(Command):
    server_only = True
    min_level = 3
    description = "<MIZA>가 서버의 메시지 이벤트를 현재 채널에 기록한다."
    usage = "(활성화|비활성화)?"
    flags = "aed"
    rate_limit = 1
...
class FileLog(Command):
    server_only = True
    min_level = 3
    description = "<MIZA>가 서버의 삭제된 파일을 현재 채널에 기록한다."
    usage = "(활성화|비활성화)?"
    flags = "aed"
    rate_limit = 1
...
class StarBoard(Command):
    server_only = True
    min_level = 2
    description = "<MIZA>가 서버에서 현재 채널로 특정 개수의 반응이 있는 인기 있는 메시지를 다시 게시한다."
    usage = "<0:반응> <1:반응_개수(1)>? <채널_활성화{?e}>? <채널_비활성화{?d}>? <채널_아이디들(-1)>*"
    flags = "ed"
    directions = [b'\xe2\x8f\xab', b'\xf0\x9f\x94\xbc', b'\xf0\x9f\x94\xbd', b'\xe2\x8f\xac', b'\xf0\x9f\x94\x84']
    dirnames = ["First", "Prev", "Next", "Last", "Refresh"]
    rate_limit = 1
...
class Crosspost(Command):
    server_only = True
    name = ["Repost", "Subscribe","재게시","교차게시"]
    min_level = 3
    description = "<MIZA>가 자동으로 대상 채널의 모든 메시지를 현재 채널로 교차 게시한다."
    usage = "<채널> <비활성화{?d}>?"
    flags = "aed"
    directions = [b'\xe2\x8f\xab', b'\xf0\x9f\x94\xbc', b'\xf0\x9f\x94\xbd', b'\xe2\x8f\xac', b'\xf0\x9f\x94\x84']
    dirnames = ["First", "Prev", "Next", "Last", "Refresh"]
...
class Publish(Command):
    server_only = True
    name = ["News", "AutoPublish","자동게시"]
    min_level = 3
    description = "<MIZA>가 현재 채널에 게시된 모든 메시지를 자동으로 게시한다."
    usage = "(활성화|비활성화)? <강제{?x}>?"
    flags = "aedx"
    rate_limit = 1
...
class CreateEmoji(Command):
    server_only = True
    name = ["EmojiCreate", "EmojiCopy", "CopyEmoji", "Emote", "Emoticon", "Emoji","이모지생성","이모지","이모지복사","이모티콘"]
    min_level = 2
    description = "URL 또는 첨부 파일에서 사용자 지정 이모지를 만든다."
    usage = "<1:이름>+ <0:url>"
    flags = "aed"
    no_parse = True
    rate_limit = (3, 6)
...
class CreateSticker(Command):
    server_only = True
    name = ["StickerCreate", "StickerCopy", "CopySticker", "Sticker","스티커","스티커생성","스티커복사"]
    min_level = 2
    description = "URL 또는 첨부 파일에서 사용자 지정 스티커를 만든다."
    usage = "<1:이름>+ <0:url>"
    flags = "aed"
    no_parse = True
    rate_limit = (3, 6)
...
class ScanEmoji(Command):
    name = ["EmojiScan", "ScanEmojis","이모지스캔"]
    min_level = 1
    description = "현재 서버의 모든 이모지에서 잠재적인 문제를 검색한다."
    usage = "<개수(inf)>"
    no_parse = True
    rate_limit = (4, 7)
    _timeout_ = 4
...
class ThreadList(Command):
    name = ["ListThreads", "Threads", "ReviveThreads", "ReviveAll","스레드","스레드부활","모두부활","스레드목록"]
    description = "현재 서버의 모든 스레드를 표시하거나 부활시킨다."
    flags = "r"
    no_parse = True
    time_consuming = True
    _timeout_ = 8

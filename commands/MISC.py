...
class CS_mem2flag(Command):
    name = ["CS_m2f","메모리ToFlag","mtf"]
    description = "특정 메모리 주소를 특정 값으로 설정하는 일련의 Cave Story TSC 명령을 반환합니다."
    usage = "<0:메모리 주소> <1:값(1)>?"
    rate_limit = 1
...
class CS_flag2mem(Command):
    name = ["CS_f2m","flagTo메모리","ftm"]
    description = "지정된 플래그 번호가 가리키는 메모리 오프셋 및 특정 비트를 반환합니다."
    usage = "<플래그>"
    rate_limit = 1
...
class CS_num2val(Command):
    name = ["CS_n2v","n2v"]
    description = "특정 문자 수 내에서 원하는 숫자를 나타내는 TSC 값을 반환합니다."
    usage = "<0:숫자> <1:길이(4)>?"
    rate_limit = 1
...
class CS_val2num(Command):
    name = ["CS_v2n","v2n"]
    description = "지정된 TSC 값으로 인코딩된 숫자를 반환합니다."
    usage = "<tsc값>"
    rate_limit = 1
...
class CS_hex2xml(Command):
    time_consuming = True
    name = ["CS_h2x","h2x","16toxml"]
    description = "주어진 Cave Story 16진수 패치를 Booster's Lab에서 읽을 수 있는 xml 파일로 변환합니다."
    usage = "<16진수값>"
    rate_limit = (3, 5)
...
class CS_npc(Command):
    name=["npc","엔피씨"]
    time_consuming = True
    description = "이름 또는 ID로 Cave Story NPC 목록에서 NPC를 검색합니다."
    usage = "<쿼리> <condensed{?c}>?"
    flags = "c"
    no_parse = True
    rate_limit = 2
...
class CS_flag(Command):
    name = ["CS_OOB", "CS_flags","플래그"]
    description = "Cave Story OOB 플래그 목록에서 메모리 변수를 검색합니다."
    usage = "<쿼리> <condensed{?c}>?"
    flags = "c"
    no_parse = True
    rate_limit = 2
...
class CS_mod(Command):
    time_consuming = True
    name = ["CS_search","모드"]
    description = "Doukutsu Club 및 Cave Story Tribute 사이트 포럼에서 항목을 검색합니다."
    usage = "<쿼리>"
    no_parse = True
    rate_limit = (3, 7)
...
class MathQuiz(Command):
    name = ["MathTest", "MQ","수학퀴즈"]
    min_level = 1
    description = "현재 채널에서 수학퀴즈를 실행한다"
    usage = "(easy 또는 hard)? <disable{?d}>?"
    flags = "aed"
    rate_limit = 3
...
class Wav2Png(Command):
    ...
    description = "입력 URL에서 wav2png을 실행합니다. 자세한 내용은 https://github.com/thomas-xin/Audio-Image-Converter을 참조하거나 직접 실행하십시오!"
...
class SpectralPulse(Command):
    _timeout_ = 150
    description = "Runs SpectralPulse on the input URL. Operates on a global queue system. See https://github.com/thomas-xin/SpectralPulse for more info, or to run it yourself! 입력 URL에서 SpectralPulse를 실행합니다. 글로벌 대기열 시스템에서 작동합니다. 자세한 내용은 https://github.com/thomas-xin/SpectralPulse을 참조하거나 직접 실행하십시오!"
    usage = "<0:search_links>"
    rate_limit = (12, 60)
    typing = True
...
class DeviantArt(Command):
    ...
    server_only = True
    min_level = 2
    description = "Subscribes to a DeviantArt Gallery, reposting links to all new posts. Deviant Art Gallery를 구독하고 모든 새 게시물에 대한 링크를 다시 게시합니다."

...
help_descriptions = fcdict((
    (None, "N/A"),
    ("main", "대부분 개별 사용자에 관련된 일반적인 명령"),
    ("string", "텍스트 기반 명령, 주로 도움을 주는 도구"),
    ("admin", "관리 기반 명령, 서버를 감독한다" ),
    ("voice", "VC를 통한 노래 재생, 변환 또는 다운로드 " ),
    ("image", "이미지, 애니메이션 및 비디오 만들기 또는 편집"),
    ("webhook", "웹훅 관련 명령 및 관리"),
    ("fun", "텍스트 기반 게임 및 기타 재미있는 기능"),
    ("owner", "오너 전용 명령, 변동성이 매우 높음"),
    ("nsfw", "작업에 안전하지 않음, 18개 이상의 채널에서만 사용 가능"),
    ("misc", "기타, 신뢰할 수 있는 서버로 제한됨"),
))


class Help(Command):
    name = ["❓", "❔", "?", "Halp", "도움말"]
    description = "사용 가능한 명령 목록을 표시하거나 명령에 대한 자세한 설명을 제공한다."
    usage = "<(명령|카테고리)>?" 
    flags = "v"
    no_parse = True
    slash = True
    ...
    async def __call__(self, bot, argv, user, message, original=None, **void):
        ...
        if comm:
            ...
            content = (
                f"[명령어 범주] {category_repr(com.catg)}\n"
                + f"[명령어 사용] {prefix}{com.parse_name()} {com.usage}\n"
                + f"[명령어 실행] {a}\n"
                + f"[명령어 효과] {com.parse_description()}\n"
                + f"[필요한 권한 레벨] {com.min_display}"
            )
            x = com.rate_limit
            if x:
            ...
            content = (
                f"아래 버튼 메뉴를 사용하여 명령을 선택하세요!\n"
                + f"관련한 명령어들의 자세한 설명을 영문으로 보고싶다면 [`mizatlas`]({bot.webserver}/mizatlas)를 확인하세요!\n"
                + f"명령어 사용법을 모르시겠다면 `{prefix}명령어사용법`을 입력해보세요!\n\n"
                + f"이 명지 디스코드 봇은 [`miza-bot`]({bot.github})을 리메이크 하여 만들었습니다!\n"
                + f"행복하고 편안한 디스코드 되시길."
            )
        ...
        catmenu = cdict(
            ...
            options=catsel,
            min_values=0,
            placeholder=category_repr(catg) if catg else "명령어 카테고리(범주)를 골라주세요...",
        )
        commenu = cdict(
            type=3,
            custom_id="\x7f1",
            options=comsel,
            min_values=0,
            placeholder=com.parse_name() if comm else "명령어를 골라주세요...",
        )
        buttons = [[catmenu], [commenu]]
        if content:
            embed.description = f"```callback-main-help-{user.id}-\n{user.display_name} 님의 도움말 요청!```" + content
        if original:
...
class HelpCommand(Command):
    name = ["HelpCommand", "명령어설명", "명령어사용법", "사용법"]
    description = "usage인 명령어 사용법에 대해 간략히 설명한다"
    # usage = "<>"
    no_parse = True
    slash = True
    ...
    async def __call__(self, bot, argv, user, message, original=None, **void):
        ...
        if " " in prefix:
            prefix += " "
        embed = discord.Embed()
        embed.set_author(name="❓ Command Help ❓", icon_url=best_url(user), url=bot.webserver)
        argv = full_prune(argv).replace("*", "").replace("_", "").replace("||", "")
        comm = None
        content = (
            f"[`|`] : <(명령|카테고리)> 처럼 괄호 안에 | 로 나눠져있을 경우 그 중 하나만 입력하면 된다는 뜻\n\n"
            + f"[`?`] : 뒤에 ?가 달린 것들은 필수로 쓰는 것이 아닌 하나 혹은 여러개를 한번에 입력하여 사용, (대개 속성을 정할때 사용한다.)\n\n"
            + f"[`(?알파벳)`] : (?알파벳)형태의 경우 (?알파벳)그대로 입력하면 해당 기능 실행 단축키역할\n\n"
            + f"[`<0:><1:>..`] : 인덱스 번호\n\n"
            + f"[`*`] : 필수 입력, (add|delete)? <0:tags>`*` <1:url>? 같은 경우 제일 처음에 나온 것 \nex) (add|delete)? 을 사용하고자 할 때 *가 붙은 것은 필수로 입력해야함"
        )
        
        if content:
            embed.description = f"```callback-main-help-{user.id}-\n{user.display_name} 님이 요청한 간략한 명령어 설명!```" + content
        if original:
            await interaction_patch(bot, original, embed=embed)
            return
        # elif getattr(message, "slash", None):
        #     await interaction_response(bot, message, embed=embed, buttons=buttons, ephemeral=True)
        try:
            await send_with_reply(channel, message, embed=embed, ephemeral=True)
...
class Hello(Command):
    name = ["👋", "Hi", "Hi!", "Hewwo", "Herro", "'sup", "Hey","안녕", "Greetings", "Welcome","반가워","Bye", "Cya", "Goodbye","잘가"]
    description = "인사말 메시지를 보낸다.(봇이 온라인 상태인지 확인하는 데 유용함)"
    usage = "<유저>?" 
    slash = True
...
class Perms(Command):
    server_only = True
    name = ["DefaultPerms", "기본권한", "ChangePerms", "Perm", "ChangePerm", "Permissions","권한",]
    description = "사용자의 권한 수준을 표시하거나 변경한다."
    usage = "<0:유저>* <1:새로운_권한>? <기본값{?d}>? <숨기기{?h}>?" 
    flags = "fhd"
    multi = True
    slash = True
...
class EnabledCommands(Command):
    ...
    name = ["EC", "Enable", "활성화"]
    min_display = "0~3"
    description = "현재 채널에서 명령 범주를 표시, 활성화 또는 비활성화한다."
    usage = "(활성화|비활성화|초기화)? <카테고리>? <서버전체(?s)> <리스트{?l}>? <숨기기{?h}>?"
    flags = "aedlhrs"
    slash = True
...
class Prefix(Command):
    name = ["ChangePrefix","접두사변경"]
    min_display = "0~3"
    description = "이 서버에 대한 miza봇 명령의 접두사를 표시하거나 변경한다."
    usage = "<새로운_접두사>? <기본값{?d}>?"
    flags = "hd"
...
class Loop(Command):
    time_consuming = 3
    _timeout_ = 12
    name = ["For", "Rep", "While","반복"]
    min_level = 1
    min_display = "1+"
    description = "명령을 루프한다. 필요한 경우 루프를 종료하려면 원본 메시지를 삭제한다."
    usage = "<0:횟수> <1:명령>+"
    rate_limit = (3, 7)
    active = set()
...
class Avatar(Command):
    name = ["PFP", "Icon","아이콘","프로필"]
    description = "사용자 또는 서버의 아바타에 대한 링크를 보낸다."
    usage = "<객체>*" 
    multi = True
    slash = True
...
class Info(Command):
    name = ["🔍", "🔎", "UserInfo", "ServerInfo", "WhoIs","유저정보","서버정보"]
    description = "대상 사용자 또는 서버에 대한 정보를 표시한다."
    usage = "<객체>* <세부적{?v}>?" 
    flags = "v"
    rate_limit = 1
    multi = True
...
class Profile(Command):
    name = ["User", "UserProfile","유저","유저정보"]
    description = "<MIZA>에서 사용자 프로필을 표시하거나 편집한다."
    usage = "(유저|설명|표준시간대|생일)? <값>? <삭제{?d}>?" 
    flags = "d"
    rate_limit = 1
    no_parse = True
    slash = True
...
class Activity(Command):
    name = ["Recent", "Log","기록","최근"]
    description = "대상 사용자의 서버 또는 채널에 대한 최근 Discord 활동을 표시한다."
    usage = "<유저>? <세부적{?v}>?" 
    flags="v"
    rate_limit = (2, 9)
    typing = True
...
class Status(Command):
    name = ["State", "Ping","상태","핑"]
    description = "봇의 현재 내부 프로그램 상태를 표시한다."
    usage = "(활성화|비활성화)?" 
    flags = "aed"
    slash = True
...
class Invite(Command):
    name = ["Website", "BotInfo", "InviteLink","웹사이트","초대링크","봇정보"]
    description = "<MIZA>의 홈페이지, github 및 invite 코드에 대한 링크와 현재 서버에 대한 초대 링크를 전송한다."
    slash = True
...
class Upload(Command):
    name = ["Filehost", "Files","파일","파일호스트"]
    description = "<MIZA>의 웹 서버 업로드 페이지에 대한 링크를 보낸다: ⟨WEBSERVER⟩/files"
    msgcmd = True
    _timeout_ = 50
...
class Reminder(Command):
    name = ["Announcement", "Announcements", "Announce", "RemindMe", "Reminders", "Remind","공지사항","리마인더"]
    description = "특정 날짜 및 시간에 대한 주의사항을 설정한다."
    usage = "<1:메세지>? <0:시간>? <긴급{?u}>? <삭제{?d}>?" 
    flags = "aeduc"
...
class Note(Command):
    name = ["Trash", "Notes","노트"]
    description = "지정된 문자열을 기록하고 작업관리 목록을 보고 편집할 수 있다."
    usage = "(수정|삭제)? <아이디|노트>?"
    flags = "aed"
    directions = [b'\xe2\x8f\xab', b'\xf0\x9f\x94\xbc', b'\xf0\x9f\x94\xbd', b'\xe2\x8f\xac', b'\xf0\x9f\x94\x84']
    dirnames = ["First", "Prev", "Next", "Last", "Refresh"]

...
class Queue(Command):
    server_only = True
    name = ["▶️", "P", "Q", "Play", "Enqueue", "Search&Play", "큐", "재생목록"]
    alias = name + ["LS"]
    description = "서버 내의 음악 재생목록을 보여주거나 음악을 재생한다."
    usage = "<유튜브 링크>* <force{?f}|budge{?b}|random{?r}|verbose{?v}|hide{?h}>*"
    flags = "hvfbrz"
    no_parse = True
...
class Playlist(Command):
    server_only = True
    name = ["DefaultPlaylist", "PL", "플레이리스트"]
    min_display = "0~2"
    description = "기본 플레이리스트에 음악을 보여준다.음악을 추가, 제거할 수 있다."
    usage = "(add 또는 remove)? <유튜브 링크>*"
    flags = "aedzf"
    directions = [b'\xe2\x8f\xab', b'\xf0\x9f\x94\xbc', b'\xf0\x9f\x94\xbd', b'\xe2\x8f\xac', b'\xf0\x9f\x94\x84']
    dirnames = ["First", "Prev", "Next", "Last", "Refresh"]
    rate_limit = 0.5
...
class Connect(Command):
    server_only = True
    name = ["📲", "🎤", "🎵", "🎶", "📴", "📛", "Summon", "J", "Join", "DC", "Disconnect", "Leave", "Yeet", "Move", "Reconnect", "연결", "커넥트"]
    # Because Rythm also has this alias :P
    alias = name + ["FuckOff"]
    description = "음성채널로 봇을 호출하거나 채널에서 내보내는 방법을 알려준다."
    usage = "<채널>?"
    rate_limit = (3, 4)
    slash = ("Join", "Leave")
...
class Skip(Command):
    server_only = True
    name = ["⏭", "🚫", "S", "SK", "FS", "CQ", "ForceSkip", "Remove", "Rem", "ClearQueue", "Clear", "스킵", "다음"]
    min_display = "0~1"
    description = "음성채널의 재생목록에서 현재 재생중인 음악을 건너뛴다."
    usage = "<음악 순번>* <force{?f}|vote{?v}|hide{?h}>*"
    flags = "fhv"
    rate_limit = (0.5, 3)
    slash = True
...
class Pause(Command):
    server_only = True
    name = ["⏸️", "⏯️", "⏹️", "Resume", "Unpause", "Stop","정지","스탑"]
    min_display = "0~1"
    description = "현재 재생중인 음악을 정지한다."
    usage = "<hide{?h}>?"
    flags = "h"
    rate_limit = (0.5, 3)
    slash = True
...
class Seek(Command):
    ...
    name = ["↔️", "Replay","찾기","시간"]
    min_display = "0~1"
    description = "현재 재생중인 음악의 시간대를 찾거나 지정한 시간대로 이동할 수 있다."
    usage = "<시간>? <hide{?h}>?"
    flags = "h"
    rate_limit = (0.5, 3)
    slash = True
...
class Dump(Command):
    ...
    alias = name + ["Dujmpö"]
    min_display = "0~1"
    description = "현재 재생목록의 데이터를 저장하거나 불러온다."
...
class AudioSettings(Command):
    ...
    def __init__(self, *args):
        ...
        self.description = "오디오 세팅을 변경한다."
        self.usage = "<값>? <volume(?v)|speed(?s)|pitch(?p)|pan(?e)|bassboost(?b)|reverb(?r)|compressor(?c)|chorus(?u)|nightcore(?n)|bitrate(?i)|loop(?l)|repeat(?1)|shuffle(?x)|quiet(?q)|stay(?t)|force_permanent(?f)|disable(?d)|hide(?h)>*"
...
class Jump(Command):
    server_only = True
    name = ["🔄", "Roll", "Next", "RotateQueue","점프"]
    min_display = "0~1"
    description = "재생목록의 다음 음악을 재생한다."
...
class Shuffle(Command):
    ...
    name = ["🔀", "Scramble","셔플"]
    min_display = "0~1"
    description = "재생목록의 순서를 랜덤으로 섞는다."
...
class UnmuteAll(Command):
    ...
    time_consuming = True
    min_level = 3
    description = "서버 내 모든 인원이 음소거 기능을 사용하지 못하도록 설정한다."
    usage = "<hide{?h}>?"
    flags = "h"
    rate_limit = 10
...
class VoiceNuke(Command):
    ...
    min_level = 3
    name = ["☢️","강퇴","추방"]
    description = "음성채널 내 모든 인원을 추방한다."
    usage = "<hide{?h}>?"
    flags = "h"
    rate_limit = 10
...
class Radio(Command):
    name = ["FM"]
    description = "MIZA 봇에서 재생가능한 라디오 방송국 라디오스트림을 https://worldradiomap.com 사이트에서 찾는다."
    usage = "<0:국가>? <2:주>? <1:도시>?"
    rate_limit = (2, 6)
    slash = True
    countries = fcdict()
...
class Party(Command):
    ...
    min_level = 2
    name = ["YTT","파티"]
    description = "가장 가까운 음성 채널에서 활동 파티 링크를 생성합니다!"
...
class Player(Command):
    ...
    min_display = "0~3"
    description = "현재 재생목록에 대한 새로운 UI의 플레이어를 제공합니다."
...
class Lyrics(Command):
    time_consuming = True
    name = ["SongLyrics","가사","노래가사"]
    description = "입력받은 유튜브 링크의 노래 가사를 검색합니다."
    usage = "<유튜브 링크>* <verbose{?v}>?"
    flags = "v"
    rate_limit = (2, 6)
    typing = True
    slash = True
...
class Download(Command):
    time_consuming = True
    _timeout_ = 75
    name = ["📥", "Search", "YTDL", "Youtube_DL", "AF", "AudioFilter", "Trim", "Concat", "Concatenate", "🌽🐱", "ConvertORG", "Org2xm", "Convert","다운로드"]
    description = "YouTube/SoundCloud 쿼리 또는 오디오 파일 링크에서 노래를 검색 및/또는 다운로드합니다."
    usage = "<0:유튜브 링크>* <multi{?m}> <trim{?t}>? <-3:trim_start|->? <-2:trim_end|->? <-1:out_format(mp4)>? <concatenate{?c}|remove_silence{?r}|apply_settings{?a}|verbose_search{?v}>*"
    flags = "avtzcrm"
    rate_limit = (7, 16)
    typing = True
    slash = True

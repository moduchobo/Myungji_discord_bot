...
class Text2048(Command):
    time_consuming = True
    name = ["2048", "🎮"]
    description = "버튼을 사용하여 2048 게임을 한다. 획득한 포인트는 금으로 보상된다."
    usage = "<0:치수_크기(4x4)>* <1:치수_개수(2)>? <공개{?p}|특별한_타일{?s}|어려운_모드{?i}|쉬운_모드{?e}>*"
    flags = "pies"
    rate_limit = (3, 9)
    reacts = ("⬅️", "➡️", "⬆️", "⬇️", "⏪", "⏩", "⏫", "⏬", "◀️", "▶️", "🔼", "🔽", "👈", "👉", "👆", "👇")
...
class Snake(Command):
    ...
    name = ["Snaek", "🐍","뱀","스네이크"]
    rate_limit = (3, 9)
    description = "버튼을 사용하여 스네이크 게임을 한다."
    usage = "<치수(8x8)>* <공개{?p}|어러운_모드{?i}>*"
    flags = "pi"
    slash = True
...
class SlotMachine(Command):
    name = ["Slots","슬롯"]
    description = "슬롯 머신 게임을 실행한다. 게임을 하는 데 금이 들고, 금과 다이아몬드를 생산할 수 있다."
    usage = "<투자_금액{50}>? <애니메이션_스킵{?s}>?"
    flags = "s"
    rate_limit = (5, 10)
    emojis = {
        "❤️": 20,
...
class Barter(Command):
    description = "마인크래프트 피글린 물물교환기를 시뮬레이션한다. 금괴를 사용한다. 자세한 내용은 ~shop 및 ~bal을 참고하라!"
    usage = "<양>"
    rate_limit = 1
...
class Uno(Command):
    description = "나랑, 아니면 친구들이랑 유노 게임해!"
    rate_limit = 2
...
class Matchmaking(Command):
    name = ["Ship","배송","매치"] + HEARTS
    description = "랜덤화된 백분율로 제공된 두 개의 개체를 배송한다."
    usage = "<객체들>*"
    slash = ("Ship",)
...
class Pay(Command):
    name = ["GiveCoins", "GiveGold","코인지급"]
    description = "대상 사용자에게 지정된 양의 코인을 지급한다."
    usage = "<0:유저> <1:양(1)>?"
    rate_limit = 0.5
...
class React(Command):
    ...
    name = ["AutoReact","자동응답"]
    min_level = 2
    description = "<MIZA>가 substring을 포함하는 메시지에 대한 응답을 자동으로 할당한다."
    usage = "<0:~에게_응답>? <1:응답_데이터>? <비활성화{?d}>?"
    flags = "aedzf"
    no_parse = True
...
class Dogpile(Command):
    server_only = True
    min_level = 2
    description = "동일한 메시지가 3개 이상 연속으로 게시될 때 <MIZA>가 사용자를 자동으로 모방한다. 작동될 떄 XP 및 골드를 부여한다."
    usage = "(활성화|비활성화)?"
    flags = "aed"
    rate_limit = 0.5
...
class Daily(Command):
    name = ["Quests", "Quest", "Tasks", "Challenges", "Dailies","퀘스트"]
    description = "일일 퀘스트 목록을 표시한다."
    rate_limit = 1
...
class Wallet(Command):
    name = ["Level", "Bal", "Balance","지갑","레벨","상태"]
    description = "대상 사용자의 지갑을 표시한다."
    usage = "<유저>*"
    rate_limit = 1
    multi = True
    slash = ("Wallet", "Bal")
...
class Shop(Command):
    description = "상점 시스템을 표시하거나 품목을 구입한다."
    usage = "<아이템[]>"
    rate_limit = 1
...
class Cat(ImagePool, Command):
    description = "thecatapi.com, api.alexflipnote.dev/cats, 또는 cdn.nekos.life/meow에서 임의 이미지를 가져와 포함한다. <WEBSERVER>/cats를 꼭 확인하라!"
    database = "cats"
    name = ["🐱", "Gato", "Meow", "Kitty", "Kitten","고양이"]
    slash = True
    http_nums = {
        100, 101, 102,
...
class Dog(ImagePool, Command):
    description = "images.dog.ceo, api.alexflipnote.dev/dogs, 또는 cdn.nekos.life/woof에서 임의의 이미지를 가져와 포함한다.<WEBSERVER>/dogs를 꼭 확인하라!"
    database = "dogs"
    name = ["🐶", "Woof", "Doggy", "Doggo", "Puppy", "Puppo","강아지","개"]
    slash = True
...
class _8Ball(ImagePool, Command):
    description = "cdn.nekos.life/8ball에서 임의의 이미지를 가져와 포함한다."
    database = "8ball"
    name = ["🎱"]
...
class XKCD(ImagePool, Command):
    description = "xkcd.com에서 임의의 이미지를 가져와 포함한다."
    database = "xkcd"
...
class Turnoff(ImagePool, Command):
    description = "turnoff.us에서 임의의 이미지를 가져와 포함한다."
    database = "turnoff"
    threshold = 1
...
class Inspiro(ImagePool, Command):
    name = ["InspiroBot"]
    description = "inspirobot.me에서 임의의 이미지를 가져와 포함한다."
    database = "inspirobot"
...
class ImageSearch(ImagePool, Command):
    name = ["🖼", "🧁", "ImgSearch", "Muffin", "Muffins","이미지검색","머핀"]
    description = "태그를 사용하여 gettyimages.co.uk 및 unsplash.com의 검색에서 임의 이미지를 가져온다."
    threshold = 9
...
class Giphy(ImagePool, Command):
    name = ["GIFSearch","GIF검색"]
    description = "태그를 사용하여 giphy.com의 검색에서 임의 이미지를 가져온다."
    threshold = 4
    sem = Semaphore(5, 256, rate_limit=1)
...
class Rickroll(Command):
    name = ["Thumbnail", "FakeThumbnail", "FakeVideo","썸네일","가짜비디오","가짜썸네일"]
    description = "썸네일 그림이 포함된 링크를 생성하지만 재생된 후에는 별도의 YouTube 비디오로 다시 보낸다."
    usage = "<썸네일>? <비디오>?"
    no_parse = True
    rate_limit = 1
...
class RPS(Command):
    name = ["Rockpaperscissors","가위바위보"]
    description = "가위바위보 랜덤 게임!"
    usage = "<바위>? <보>? <가위>?"
    slash = True
    typing = False
...
class Akinator(Command):
    name = ["Aki","아키"]
    description = "실제 인물이나 허구 인물에 대해 생각해 봐. 누군지 맞춰볼게."
    usage = "<언어(en)>? <어린이_친화적인{?c}>"
    flags = "c"
    slash = True
    rate_limit = (1, 3)
    session = None
